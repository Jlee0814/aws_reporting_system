...
//    private RestTemplate rs;
    ...
    private ReportRequestEntity persistToLocal(ReportRequest request) {
        ...
        ReportRequestEntity entity = null;
        if(request.getReqId()==null) {
            request.setReqId("Req-" + UUID.randomUUID().toString());
            entity = new ReportRequestEntity();
        }else{
            entity = reportRequestRepo.findById(request.getReqId()).orElseThrow(RequestNotFoundException::new);
        }
...
//        }
        // no need to try catch there's fallback for these two method call
        PDFResponse pdfResponse  = pdfClient.createPDF(request).getBody();
        ExcelResponse excelResponse  = excelClient.createExcel(request).getBody();
        List<Object> parallel = Arrays.asList(excelResponse,pdfResponse);
        parallel.parallelStream().forEach(this::updateLocal);
    }
...
//    }
    private void updateLocal(Object reportResp) {
        SqsResponse response = new SqsResponse();
        BeanUtils.copyProperties(reportResp, response);
        System.out.println("before update");
        if(reportResp instanceof PDFResponse){
            System.out.println("update pdf");
            updateAsyncPDFReport(response);
        }else if(reportResp instanceof ExcelResponse){
    ...
    public void updateAsyncPDFReport(SqsResponse response) {
        System.out.println("updateAsyncPDFReport");
...
//  @Transactional
    public void updateAsyncExcelReport(SqsResponse response) {
        System.out.println("updateAsyncExcelReport");
        ReportRequestEntity entity = reportRequestRepo.findById(response.getReqId()).orElseThrow(RequestNotFoundException::new);
...
//            InputStream is = restTemplate.execute(, HttpMethod.GET, null, ClientHttpResponse::getBody, fileId);
    ...
    public void removeReportById(String reqId){
        //remove pdf in s3 bucket
        ReportRequestEntity entity = reportRequestRepo.findById(reqId).orElseThrow(RequestNotFoundException::new);
        String fileLocation = entity.getPdfReport().getFileLocation(); // this location is s3 "bucket/key"
        ...
        if(fileLocation!=null) {
            String bucket = fileLocation.split("/")[0];
            String key = fileLocation.split("/")[1];
            if(s3Client.getObject(bucket,key)!=null)//avoid null pointer exception
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateReportById(ReportRequest request){
        //remove old pdf in s3 bucket
        ...
        //updating an file also have to generate the new PDF and Excel;
        generateReportsSync(request);
    }
}
