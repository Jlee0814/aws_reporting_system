...
//   @DeleteMapping
    ...
    public ResponseEntity<GeneralResponse> removeReportById(@PathVariable String reqId) {
        log.info("Delete Report By Id ", reqId);
        //delete from s3 bucket first then delete from database
        reportService.removeReportById(reqId);
        return ResponseEntity.ok(new GeneralResponse("delete successfully"));
    }
//   @PutMapping
    @PutMapping("/report")
    public  ResponseEntity<GeneralResponse> updateReportById(@RequestBody @Validated ReportRequest request) {
        log.info("update Report By Id ", request);
        reportService.updateReportById(request);
        return ResponseEntity.ok(new GeneralResponse("update successfully"));
    }
